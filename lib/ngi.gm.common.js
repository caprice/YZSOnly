...
gm.ngi.Dialog.Tip = function(option, config) {
	this.setting = {
		outer : [
				...
				'<div id="msgArrBoxS"><img width="165" height="165" src="materials/22.png"></div><!---这里程序修改图片的SRC--->',
...
gm.ngi.Dialog.Load = function(option, config) {
	...
	this.setting = {
		outer : [
				...
				'<div id="msgArrBox_left"></div>',
				'<div id="msgArrBox_middle">',
				'<div id="msgArrBoxS_loading"><img width="83" height="83" src="materials/26.png"></div><!---这里程序修改图片的SRC--->',
				'<div id="contentBoxS">{0}</div>', '<span></span>', '</div>',
				'<div id="msgArrBox_right"></div>', '</div>' ].join(""),
		lurk : true,
		zIndex : zi,
...
gm.ngi.Dialog.Confirm = function(option) {
	...
	this.setting = {
		outer : [ '<div id="msgBoxB" et="contentContainer">',
				...
				'<li et="cancel">' + option.btn2 + '</li>', '</ul>', '</div>',
				'</div>' ].join(),
		lurk : false,
		zIndex : zi,
		onShow : this.showing,
...
$unit.ns("gm.ngi.weibo.Utils");
gm.ngi.weibo.Utils = function() {
...
gm.ngi.weibo.utils = new gm.ngi.weibo.Utils();

// TODO: 删除
function writeLog(s){
	console.log("*** Sina Weibo Application Log => ***"); 
	console.log("*** ==== " + s);
}

function writeWarn(s){
	console.log("*** Sina Weibo Application Warn => ***"); 
	console.warn("*** ==== " + s);
}

function writeError(s){
	console.log("*** Sina Weibo Application Error => ***"); 
	console.error("*** ==== " + s);
}